...
function log_status
{
       ...
       if [ "$status" -ne 0 ]; then
                ...
                mail -a /home/$USER/logging/"AbstractActlImport_log" -s "This is the failed job log" ravikiran.dasar@mmiholdings.co.za,warren.roux@mmiholdings.co.za < /home/$USER/logs/${TIMESTAMP}.fail_log
                #exit 1
                else
                    echo "`date +\"%Y-%m-%d %H:%M:%S\"` [INFO] $message [Status] $status : success" | tee -a "${success_logs}"
                   mail  -a /home/$USER/logging/"AbstractActlImport_log" -s "This is the Success job log " ravikiran.dasar@mmiholdings.co.za < /home/$USER/logs/${TIMESTAMP}.success_log
                fi
}
